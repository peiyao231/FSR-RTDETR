...
from ultralytics.nn.modules.block import SNI
from .AddModules import *
from .AddModules.GoldYolo import *
from .AddModules import LGAG_CSFPR
# from ultralytics.nn.AddModules.FBRT_YOLO import (
#     Blocks as FERT_Blocks,              # 重命名为 FERT_Blocks
#     ConvNormLayer as FERT_ConvNormLayer # 重命名为 FERT_ConvNormLayer
# )
# from ultralytics.nn.AddModules.FBRT_YOLO import Blocks as FERT_Blocks, ConvNormLayer as FERT_ConvNormLayer
import contextlib
import pickle
import re
import types
...
+        # -------------------------
+        # Custom hooks: FSAS / LGF
+        # -------------------------
+        if m is FSAS:
+            # FSAS is channel-preserving: out = in
+            c1 = ch[f]
+            c2 = c1
+            args = [c1, *args]
+
+        elif m is LGF:
+            # LGF takes two inputs: (x_low, x_high)
+            # f should be a list/tuple like [low_idx, high_idx]
+            c_low, c_high = ch[f[0]], ch[f[1]]
+            c2 = args[0]
+            if c2 != nc:
+                c2 = make_divisible(min(c2, max_channels) * width, 8)
+            # LGF(in_features=[c_low,c_high], out_features=c2, groups=...)
