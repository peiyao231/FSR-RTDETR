...
from ultralytics.MTAS.semantic_attrs import build_attr_matrices, CLS_NAMES
...
#------------------ 语义监督的权重（先设小一点）-----------------
        # self.lambda_color = 0.1
        # self.lambda_shape = 0.1
        # self.lambda_content = 0.1



        if loss_gain is None:
            # loss_gain = {"class": 1, "bbox": 6, "giou": 2.4, "no_object": 0.1, "mask": 1, "dice": 1}#改进损失函数版本
            loss_gain = {"class": 1, "bbox": 5, "giou": 2, "no_object": 0.1, "mask": 1, "dice": 1}#原版
        self.nc = nc
...
# ------------------语义约束版loss(TT100K)-----------------
        ...
        # # ====== 先算原来的分类 + bbox 损失 ======
        # losses = {}
        # losses.update(self._get_loss_class(pred_scores, targets, gt_scores, len(gt_bboxes), postfix))
        # losses.update(self._get_loss_bbox(pred_bboxes, gt_bboxes, postfix))

        # # ====== 再算语义属性损失（MTAS） ======
        # # 只在类别数 = TT100K 45 类时启用，避免影响 CCTSDB
        # num_classes = pred_scores.shape[-1]
        # use_semantic = (num_classes == len(CLS_NAMES)) and len(gt_bboxes) > 0

        # if use_semantic:
        #     # 正样本 logits: [N_pos, C]
        #     src_logits = pred_scores[idx]        # 和上面 targets[idx] 对应
        #     target_classes_o = gt_cls[gt_idx].long()  # [N_pos] 细类ID

        #     # 类概率
        #     pos_prob = src_logits.softmax(-1)    # [N_pos, C]

        #     # 属性矩阵
        #     info = build_attr_matrices(device=src_logits.device)
        #     color_mat   = info["color_mat"]      # [C, Nc]
        #     shape_mat   = info["shape_mat"]      # [C, Ns]
        #     content_mat = info["content_mat"]    # [C, Nt]
        #     color_ids   = info["color_ids"]      # [C]
        #     shape_ids   = info["shape_ids"]
        #     content_ids = info["content_ids"]

        #     # 每个 GT 类对应的属性 ID: [N_pos]
        #     tgt_color   = color_ids[target_classes_o]
        #     tgt_shape   = shape_ids[target_classes_o]
        #     tgt_content = content_ids[target_classes_o]

        #     # 类概率 -> 属性概率: [N_pos, C] @ [C, N*] -> [N_pos, N*]
        #     pred_color_prob   = pos_prob @ color_mat
        #     pred_shape_prob   = pos_prob @ shape_mat
        #     pred_content_prob = pos_prob @ content_mat

        #     n_pos = pos_prob.shape[0]
        #     idx_range = torch.arange(n_pos, device=src_logits.device)

        #     color_p   = pred_color_prob[idx_range, tgt_color].clamp(min=1e-6)
        #     shape_p   = pred_shape_prob[idx_range, tgt_shape].clamp(min=1e-6)
        #     content_p = pred_content_prob[idx_range, tgt_content].clamp(min=1e-6)

        #     loss_color   = -color_p.log().mean()
        #     loss_shape   = -shape_p.log().mean()
        #     loss_content = -content_p.log().mean()
        # else:
        #     # 非 TT100K 或当前 batch 没有正样本时，三个 loss 置 0
        #     zero = pred_scores.sum() * 0.0
        #     loss_color = loss_shape = loss_content = zero

        # # 加入到 loss 字典（带上 postfix，兼容 _dn）
        # losses[f"loss_color{postfix}"]   = self.lambda_color   * loss_color
        # losses[f"loss_shape{postfix}"]   = self.lambda_shape   * loss_shape
        # losses[f"loss_content{postfix}"] = self.lambda_content * loss_content
